...
## Rotation and Android Back fixes

### Search state on orientation changes

The downloaded search-result list is now owned by `ArxivSearchViewModel`.
Android retains this ViewModel when the Activity is recreated during a
portrait/landscape transition, so the current paper results are not cleared.

The existing keyword, keyword mode, period, maximum-results setting, status,
and list position remain saveable Compose state. The selected top-level tab
also survives rotation.

A ViewModel is used instead of putting the complete paper list into the
saved-instance-state Bundle because papers can contain large abstracts.

### System Back in the in-app reader

While the in-app WebView is visible, Android's system Back action is intercepted
with Compose `BackHandler` and closes the reader first. It no longer exits the
whole application from the reader.

The WebView URL/title are also saveable so an orientation change does not
silently drop the reader overlay.

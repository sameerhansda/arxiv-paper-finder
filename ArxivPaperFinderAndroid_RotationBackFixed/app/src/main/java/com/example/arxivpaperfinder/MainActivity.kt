...
class ArxivSearchViewModel : ViewModel() {
...
fun ArxivApp(
    ...
    val context = androidx.compose.ui.platform.LocalContext.current
    val searchViewModel: ArxivSearchViewModel = viewModel()
    val prefs = remember {
    ...
    // Horizontal swipe navigation between Papers and Bookmarks.
    // Positive drag = finger moves right; negative drag = finger moves left.
    var tabHorizontalDrag by remember {
    ...
    val searchPapers = searchViewModel.papers
    ...
    var inAppWebUrl by rememberSaveable {
        mutableStateOf<String?>(null)
    }

    var inAppWebTitle by rememberSaveable {
                ...
                modifier = Modifier
                    ...
                    .pointerInput(currentTab) {
                        detectHorizontalDragGestures(
                            onDragStart = {
                                tabHorizontalDrag = 0f
                            },
                            onHorizontalDrag = { change, dragAmount ->
                                tabHorizontalDrag += dragAmount
                                change.consume()
                            },
                            onDragEnd = {
                                val thresholdPx = 72.dp.toPx()

                                when {
                                    currentTab == "Papers" &&
                                        tabHorizontalDrag < -thresholdPx -> {
